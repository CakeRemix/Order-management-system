...
\begin{itemize}[leftmargin=2em, itemsep=6pt]
    \item ❌ \textbf{Using synchronous bcrypt calls:} Blocks event loop, creates performance bottlenecks.
    \item ✅ \textbf{Our approach:} Async/await with promise-based API.
    
    \item ❌ \textbf{Comparing passwords with === operator:} Vulnerable to timing attacks.
    \item ✅ \textbf{Our approach:} bcrypt.compare() uses constant-time comparison.
    
    \item ❌ \textbf{Setting cost factor too low (<10):} Insufficient protection against modern hardware.
    \item ✅ \textbf{Our approach:} Cost factor 12 calibrated to current threat landscape.
    
    \item ❌ \textbf{Logging passwords or hashes:} Creates inadvertent security exposure.
    \item ✅ \textbf{Our approach:} Log authentication events only, never credentials.
    
    \item ❌ \textbf{Allowing unlimited password length:} DoS vulnerability via excessive computation.
    \item ✅ \textbf{Our approach:} 64-character maximum prevents abuse.
\end{itemize}
...
\hline
Level 1: Manual & Manual testing, deployment & Legacy enterprises & ❌ Surpassed \\
\hline
Level 2: Basic CI & Automated builds, tests & Small startups & ❌ Surpassed \\
\hline
Level 3: Automated CD & Automated deployment to staging & Mid-size companies & ✅ Achieved \\
\hline
Level 4: Full CD & Prod deployment automation & Netflix, Spotify & 🔄 In progress \\
\hline
Level 5: Self-Service & Dev-controlled deployments & Google, Meta & ⏳ Future goal \\
\hline
\end{tabular}
...
\begin{itemize}[leftmargin=2em, itemsep=6pt]
    \item ❌ \textbf{Over-engineering initially:} Complex pipelines with every feature upfront create maintenance burden.
    \item ✅ \textbf{Our approach:} MVP pipeline first, add features based on actual pain points.
    
    \item ❌ \textbf{Ignoring pipeline performance:} Slow pipelines discourage frequent commits and CI usage.
    \item ✅ \textbf{Our approach:} 8-minute target with aggressive caching and parallelization.
    
    \item ❌ \textbf{Weak test coverage requirements:} Low coverage thresholds allow quality regression.
    \item ✅ \textbf{Our approach:} 70\% minimum coverage, 80\% for new code, enforced by pipeline.
    
    \item ❌ \textbf{Manual approval for everything:} Excessive gates slow delivery without improving quality.
    \item ✅ \textbf{Our approach:} Automated staging deployment, manual approval only for production.
    
    \item ❌ \textbf{Insufficient monitoring:} Deployments succeed but application broken in production.
    \item ✅ \textbf{Our approach:} Health checks, smoke tests, and error rate monitoring post-deployment.
\end{itemize}
