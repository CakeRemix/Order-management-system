...
\subsection{Overview}
\small The Food Truck Order Management System is a simple website for ordering food from the food trucks on the GIU campus. It helps to reduce waiting in lines by letting users place orders before going to the truck.

\textbf{Who uses it:}
\begin{itemize}[leftmargin=1.8em,itemsep=4pt]
  \item \textbf{Students and Staff (Customers):} \small Browse trucks, view menus, select items, place an order, choose (or accept) a pickup time, and check the order status (received, preparing, ready) on the website.
  \item \textbf{Food Truck Staff:}\small See incoming orders, update each order’s status, manage menu items (add, edit, mark unavailable).
  \item \textbf{Administrator:}\small Add or remove food trucks and oversee basic platform usage.
\end{itemize}
...
\item \textbf{Authentication:}  currently, none
\item \textbf{Payement:}  currently, none
\item \textbf{Hosting:}  currently, none
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{noo.png}
    \caption{Client Use Case}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{noa.png}
    \caption{Restaurant Use Case}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{noe.png}
