...
\begin{itemize}[leftmargin=2em, itemsep=6pt]
    ...
    \item \textbf{Menu Browsing:} Users can browse their menu.
    ...
    \item \textbf{Edit or Cancel Orders:} Orders can be edited or cancelled only if still in \textit{Pending} status.
    \item \textbf{6-Digit Pickup Code:} A unique 6-digit code is generated to verify the customer at pickup.
    \item \textbf{Busy Mode Notification:} Customers are notified if a vendor is currently too busy to accept new orders.
    \item \textbf{Bestseller Items Display:} Popular items are highlighted based on order frequency.
    \item \textbf{Leave Reviews:} Customers can write reviews after order completion.
\end{itemize}
...
\begin{itemize}[leftmargin=2em, itemsep=6pt]
    ...
    \item \textbf{Menu Management:} Vendors can add, edit, or remove menu items.
    \item \textbf{Activate Busy Mode:} Vendors can temporarily stop accepting new orders.
    \item \textbf{Track Popular Items:} Vendors can view which items are bestsellers.
    \item \textbf{View Customer Feedback:} Vendors can see customer reviews for insights and improvement.
\end{itemize}

\subsubsection{Admin Features}
\begin{itemize}[leftmargin=2em, itemsep=6pt]
    \item \textbf{Inventory Monitoring:} Admin can track stock and remove out-of-stock items.
    \item \textbf{Account Suspension:} Admin has the authority to suspend vendor accounts if needed.
    \item \textbf{Platform Analytics:} Admin can view statistics related to orders, vendors, and system usage.
\end{itemize}
...
\begin{itemize}[leftmargin=2em, itemsep=6pt]
    ...
    \item \textbf{Push Notifications:} Real-time push notifications (e.g., mobile or browser alerts) are not supported in this release.
\item \textbf{Multiple Language Support:} The system will only support English in its initial release. Support for other languages is excluded from scope.
 ...
 \item \textbf{Real-Time Communication :} The system will use simple periodic refresh or polling to check order updates. Advanced real-time communication technologies are not included in this version.
...
\begin{itemize}[leftmargin=2em, itemsep=8pt]
    \item \textbf{Operating Hours:} System functionality is limited to food truck operational hours (7:00 AM - 5:00 PM for providers, 8:00 AM - 4:40 PM for customers, Saturday to Thursday).
...
\begin{itemize}[leftmargin=2em, itemsep=8pt]
    ...
    \item \textbf{Real-Time Requirements:} Order status updates operate on polling or periodic refresh mechanisms, not true real-time WebSocket connections.
